...
const fs = require('fs')
let chatVoiceOneClient = (webSocketServer) => {
                ...
                if (jsonData.type_data === typeData.USER_SEND_MESSAGE) {
                    ...
                    if (jsonData.hasOwnProperty("token")) {
                        ...
                        if (jsonData.type_chat === "user") {
                            if (message.type === typeData.MESSAGE_TYPE_OFF_AUDIO) {
                                var isSend = false
                                webSocketServer.clients.forEach(function each(client) {
                                    if (client.hasOwnProperty('user') && client.readyState === client.OPEN
                                        && client.user.user_id === message.receiver_id) {
                                        console.log("send Voice :" + client.user.user_id, client.readyState, client.OPEN)
                                ...
                                if (isSend === false) {
                                    userController.getUserClientById(message.receiver_id, (rs) => {
                                        notification.sendOneDevice(rs.token_client,
                                            typeNotification.notice(typeNotification.TITLE_MESSAGE, typeNotification.NOTIFICATION_MESSAGE, rs.fullname, typeNotification.MESSAGE_VOICE, user.user_id))
                                    })
                                }
                                message.sender_id = user.user_id
                                message.create_date = require('moment')().format('YYYY-MM-DD HH:mm:ss')

                                let nameFile = getFilenameVoice(socket.user.user_id)
                                writeFileVoice(nameFile.path, message.content)
                                ...
                                msgController.sendMessageById(message, (rs) => {
                                    if (rs.code === typeData.USER_SEND_MESSAGE) {
                                        ...
                                        console.log("message voice: ", 'đã gửi')
                                    }
                                            ...
                                            if (member.user_id != user.user_id) {
                                                ...
                                                //    socketIsConnected(member.user_id).send(message.content)
                                                    ...
                                                    userController.getUserClientById(member.user_id, (u) => {
                                                        console.log("user memmber", u)
                                                        if (u.token_client) {
                                                            notification.sendOneDevice(u.token_client,
                                                                typeNotification.notice(typeNotification.TITLE_MESSAGE, typeNotification.NOTIFICATION_MESSAGE, u.fullname, typeNotification.MESSAGE_VOICE, user.user_id))
                                                        }
                                                    })
                                                }
                                            }
...
function writeFileVoice(fileName, dataBase64) {
    fs.writeFile(fileName, dataBase64, 'base64', function (err) {
        console.log(err);
    });
}

function getFilenameVoice(userId) {
    let now = new Date().getTime()
    let nameFile = `${userId}${now}`;
    return {
        path: `public/voice/${nameFile}.wav`,
        path_save: `/voice/${nameFile}.wav`
    }
}

module.exports = {
    chatVoiceOneClient: chatVoiceOneClient
}